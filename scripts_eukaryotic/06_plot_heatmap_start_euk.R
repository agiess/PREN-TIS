...
start_m5_50 <- subset(start_m5, V1<=20 & V1 >= -40);
...
start_m3_50 <- subset(start_m3, V1<=45 & V1 >= -15);
...
mLen5<-ggplot(subset(start_m5_50, V2 <= 35 & V2 >= 20 & V1 <= 20 & V1 >= -40) , aes(x=V1, y=V2, fill=LenZ)) + geom_tile()  +
               ...
               scale_x_continuous(limits = c(-41,21), expand = c(0, 0), breaks=c(-40, -30, -20, -10, 0, 10, 20)) +
               scale_y_continuous(limits = c(19,36), expand = c(0, 0), breaks=c(20, 25, 30, 35)) +
               theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
               theme(text = element_text(size = 12)) +
               theme(panel.background=element_rect(fill="black"))

mLen3<-ggplot(subset(start_m3_50, V2 <= 35 & V2 >= 20 & V1 <= 45 & V1 >= -15) , aes(x=V1, y=V2, fill=LenZ)) + geom_tile()  +
               scale_fill_gradientn(colours=c("grey", "grey", "white", "white", "red1", "red2", "red3", "red3"), values=c(-4,-2,0,2,4,6,8,10), rescaler = function(x,...) x, oob = identity, name="Z-score by fragment length") +
               xlab("Position relative to start codon") + ylab("Fragment length") +
               scale_x_continuous(limits = c(-16,46), expand = c(0, 0), breaks=c(-10, 0, 10, 20, 30, 40)) +
               scale_y_continuous(limits = c(19,36), expand = c(0, 0), breaks=c(20, 25, 30, 35)) +
               theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
               theme(text = element_text(size = 12)) +
               theme(panel.background=element_rect(fill="black"))

mPos5<-ggplot(subset(start_m5_50, V2 <= 35 & V2 >= 20 & V1 <= 20 & V1 >= -40) , aes(x=V1, y=V2, fill=PosZ)) + geom_tile()  +
...
mPos3<-ggplot(subset(start_m3_50, V2 <= 35 & V2 >= 20 & V1 <= 45 & V1 >= -15) , aes(x=V1, y=V2, fill=PosZ)) + geom_tile()  +
               scale_fill_gradientn(na.value = "black", colours=c("grey", "grey", "white", "white", "red1", "red2", "red3", "red3"), values=c(-3,-2,-1,0,1,2,3,4,5,6,7), rescaler = function(x,...) x, oob = identity, name="Z-score by position") +
               xlab("Position relative to start codon") + ylab("Fragment length") +
               scale_x_continuous(limits = c(-16,46), expand = c(0, 0), breaks=c(-10, 0, 10, 20, 30, 40)) +
               scale_y_continuous(limits = c(19,36), expand = c(0, 0), breaks=c(20, 25, 30, 35)) +
...
subset_mono5.raw<-filter(mono.start5.raw, V1 >=-40, V1 <= 20)
subset_mono5.raw <- mutate(subset_mono5.raw, proportion = V2/sum(V2))

subset_mono3.raw<-filter(mono.start3.raw, V1 >=-15, V1 <= 45)
